...
function extractBookshelfID(reference: string): string | null {
  const bookshelfMatch = reference.match(/NBK\d+/i);
  return bookshelfMatch ? bookshelfMatch[0] : null;
}

function extractPMCID(reference: string): string | null {
  const pmcMatch = reference.match(/PMC\d+/i);
  return pmcMatch ? pmcMatch[0] : null;
}
...
interface ParsedReference {
  ...
  pmcid: string | null;
  source: 'pubmed' | 'cochrane' | 'europepmc' | 'semanticscholar' | 'clinicaltrials' | 'openalex' | 'pmc' | 'dailymed' | 'aap' | 'doi' | 'fdafaers' | 'openfda' | 'ada' | 'who' | 'nice' | 'acc' | 'cdc' | 'pubmedsearch' | 'fda' | 'nejm' | 'jama' | 'lancet' | 'bmj' | 'aha' | 'nature' | 'bookshelf';
...
function parseReference(reference: string): ParsedReference {
  // Leading journals list (high impact factor)
  const leadingJournals = [
    'Lancet', 'NEJM', 'New England Journal of Medicine', 'JAMA', 'BMJ', 
    'Nature', 'Science', 'Cell', 'Nature Medicine', 'Nature Reviews',
    'Annals of Internal Medicine', 'Circulation', 'Journal of Clinical Oncology',
    'Cochrane Database', 'Cochrane Database Syst Rev', 'Pediatr Ann'
  ];
  
  // Extract components first
  const doi = extractDOI(reference);
  const pmid = extractPMID(reference);
  const bookshelfId = extractBookshelfID(reference);
  const pmcid = extractPMCID(reference);
  
  // Validate reference - must have minimum content
  // Allow references without PMID/DOI (e.g., FDA FAERS, DailyMed, guidelines, NCBI Bookshelf)
  const trimmedRef = reference.trim();
  const refLowerForValidation = trimmedRef.toLowerCase();
  const isFDASource = refLowerForValidation.includes('fda faers') || 
                      refLowerForValidation.includes('openfda') ||
                      refLowerForValidation.includes('dailymed') ||
                      refLowerForValidation.includes('fda/openfda') ||
                      refLowerForValidation.includes('source: fda');
  
  const isValid = trimmedRef.length > 10 && 
                  (doi !== null || pmid !== null || bookshelfId !== null || pmcid !== null || isFDASource || trimmedRef.length > 20) && // Allow FDA sources, bookshelf, or longer refs
                  !trimmedRef.match(/^[\d\s]+$/) && // Not just numbers
                  !trimmedRef.match(/^[A-Za-z]\s*$/); // Not just single letters
  
  // Detect evidence source based on content
  let source: ParsedReference['source'] = 'doi';
  const refLower = reference.toLowerCase();
  
  if (bookshelfId) {
    source = 'bookshelf';
  } else if (refLower.includes('fda faers') || refLower.includes('faers')) {
    source = 'fdafaers';
  } else if (refLower.includes('openfda') || refLower.includes('fda/openfda')) {
    source = 'openfda';
  } else if (refLower.includes('cochrane') || refLower.includes('cochrane database')) {
    source = 'cochrane';
  } else if (refLower.includes('american academy of pediatrics') || refLower.includes('aap') || refLower.includes('pediatrics policy')) {
    source = 'aap';
  } else if (refLower.includes('dailymed') || refLower.includes('nlm.nih.gov/dailymed')) {
    source = 'dailymed';
  } else if (refLower.includes('clinicaltrials.gov') || refLower.includes('nct')) {
    source = 'clinicaltrials';
  } else if (refLower.includes('europe pmc') || refLower.includes('europepmc')) {
    source = 'europepmc';
  } else if (refLower.includes('semantic scholar')) {
    source = 'semanticscholar';
  } else if (refLower.includes('openalex')) {
    source = 'openalex';
  } else if (refLower.includes('pmc') && !refLower.includes('pubmed')) {
    source = 'pmc';
  } else if (pmid) {
    source = 'pubmed';
  }
  ...
  // First, clean the reference by removing citation markdown format, DOI and PMID
  let cleanRef = reference
    // Remove citation markdown format: [[N]](URL) at the beginning
    .replace(/^\[\[\d+\]\]\([^\)]+\)\s*/g, '')
  ...
  const yearMatch = cleanRef.match(/\(?(19\d{2}|20\d{2})\)?/);
  year = yearMatch ? yearMatch[1] : '';
  
  // IMPROVED PARSING: Try multiple strategies
  
  // Strategy 1: Look for common citation patterns
  // Pattern: "Title. Authors. Journal Year;Volume:Pages"
  // Pattern: "Authors. Title. Journal Year"
  // Pattern: "Title (Year) Journal"
  
  // Split by periods, filtering out empty parts and very short parts
  const parts = cleanRef.split(/\.\s+/).filter(p => p.trim().length > 2);
  
  if (parts.length >= 1) {
    // Find the longest part that looks like a title (usually the actual title)
    // Titles are typically longer and don't contain author patterns
    ...
    for (let i = 0; i < Math.min(parts.length, 3); i++) {
      ...
      // Skip if it looks like authors (contains "et al" or multiple commas with short segments)
      const looksLikeAuthors = 
        part.includes('et al') ||
        (part.split(',').length >= 3 && part.length < 100) ||
        part.match(/^[A-Z][a-z]+\s+[A-Z]{1,2}(,|$)/); // "Smith J," pattern
      
      // Skip if it looks like journal info (contains year with volume/pages)
      const looksLikeJournal = part.match(/\d{4};\d+/) || part.match(/\d{4}\s*\(\d+\)/);
      
      if (!looksLikeAuthors && !looksLikeJournal && part.length > bestTitleCandidate.length) {
        // Prefer parts that start with capital letter and have multiple words
        if (part.match(/^[A-Z]/) && part.split(/\s+/).length >= 3) {
          bestTitleCandidate = part;
          bestTitleIndex = i;
        } else if (bestTitleCandidate.length === 0) {
          bestTitleCandidate = part;
          bestTitleIndex = i;
        }
      }
    ...
    // Use the best title candidate
    if (bestTitleCandidate.length > 10) {
      title = bestTitleCandidate;
      
      // Check if title is just a source identifier (e.g., "National Institutes of Health: PMC1198")
      // These are not real titles and should be improved
      ...
      // Try to extract authors from parts before the title
      if (bestTitleIndex > 0) {
        const authorPart = parts[bestTitleIndex - 1].trim();
        if (authorPart.includes('et al') || authorPart.split(',').length >= 2) {
          authors = authorPart.replace(/,?\s*et al\.?/i, ' et al').trim();
        }
      }
      
      // Try to extract journal from parts after the title
      for (let i = bestTitleIndex + 1; i < parts.length; i++) {
        const part = parts[i].trim();
        if (part.match(/\d{4}/) || part.match(/;?\d+(\(\d+\))?:/)) {
          journal = part.replace(/\(?\d{4}\)?.*$/, '').trim();
          break;
        }
      }
    } else {
      // Fallback: use the entire cleaned reference as title
      title = cleanRef.replace(/\(?\d{4}\)?.*$/, '').trim();
      
      // Check if this is just a source identifier
      const isSourceIdentifier = title.match(/^(National Institutes? of Health|PubMed Central|PMC|Nature|Science|JAMA|BMJ|Lancet):\s*(PMC|NBK|S)?\d+/i);
      if (isSourceIdentifier) {
        const sourceMatch = title.match(/^([^:]+):\s*(.+)$/);
        if (sourceMatch) {
    ...
    // Single part or no parts - use cleaned reference
    title = cleanRef;
  }
  ...
  // If title is still too short or just numbers/letters, mark as invalid
  if (title.length < 10 || title.match(/^[\d\s]+$/) || title.match(/^[A-Za-z]$/)) {
    // Try to use the full reference as title (last resort)
    title = cleanRef.substring(0, 150).replace(/\(?\d{4}\)?.*$/, '').trim();
  }
  
  // Extract volume and pages from journal string
  const volumeMatch = journal.match(/;?\s*(\d+)\s*(?:\(|:)/);
  volume = volumeMatch ? volumeMatch[1] : '';
  
  const pagesMatch = journal.match(/:([0-9\-e]+)/i);
  ...
  // Check if leading journal
  const isLeadingJournal = leadingJournals.some(lj => 
    journal.toLowerCase().includes(lj.toLowerCase()) ||
    reference.toLowerCase().includes(lj.toLowerCase())
  );
  
  // Check if systematic review or Cochrane
  const isSystematicReview = refLower.includes('systematic review') || 
                             refLower.includes('meta-analysis') ||
                             refLower.includes('cochrane');
  
  // Check if open access
  const isOpenAccess = refLower.includes('open access') || 
...
function parseQAResponse(response: string): {
  ...
  for (const line of lines) {
    ...
    // Check for Follow-Up Questions section (multiple variations)
    ...
    if (currentSection === 'main') {
      ...
      // Extract reference (remove leading numbers/bullets)
      ...
      // Extract question (remove leading bullets and numbers)
      ...
      // 2. If question contains citations, it's malformed - extract just the question part
      ...
      // 3. Only add if it's a proper question (ends with ?) and we haven't reached the limit
...
function LearnMoreCapabilities({ 
  ...
  const capabilities = [
    ...
    {
      title: "Clinical Workup & Lab Interpretation",
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      questions: [
        "List lab workup for chronic kidney disease stage 3b.",
        "Work up for hypercalciuria in recurrent nephrolithiasis.",
        "What diagnostic tests are indicated for suspected pulmonary embolism?"
      ]
    },
    {
      title: "Primary Evidence & Trial Data",
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
        </svg>
      ...
      questions: [
        "What is the primary evidence supporting the use of metformin as a first-line treatment for Type 2 diabetes?",
        "Provide primary trial evidence for the use of ARNI versus ACE inhibitor in heart failure with reduced ejection fraction.",
        "What randomized controlled trials support the use of low-dose aspirin for primary prevention of cardiovascular events?"
      ]
    },
    {
      ...
      questions: [
        "Write an exam question that evaluates the choice of antibiotics for pediatric otitis media.",
        "Create a 5-question mock exam on heart failure management with answers and explanations.",
        "Quiz me on the differential diagnosis of chest pain in emergency medicine."
      ]
    },
    {
      title: "Pediatric Medicine (AAP Guidelines)",
      icon: (
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      questions: [
        "What is the recommended treatment for a 7-year-old with Giardia lamblia infection?",
        "Summarize AAP guidelines for managing pediatric fever without source.",
        "What are the current AAP recommendations for childhood immunization schedules?"
      ]
    }
  ];
      ...
      <main className={`flex-1 px-6 ${hasSubmittedQuery ? 'py-6' : 'flex items-center justify-center py-12'}`}>
        ...
        >
          ...
          <div className="relative">
            ...
            {/* Main Search Bar */}
            <div
              className={`relative bg-white border-2 rounded-full shadow-lg transition-all duration-300 focus-within:border-blue-400 ${
                isDragging ? "border-blue-400 shadow-blue-100" : "border-gray-200 hover:border-blue-300"
              }`}
